...
public class SJF extends JFrame {
    ...
    public SJF() {
        setTitle("SJF Scheduling with Arrival Time (Animated)");
        setSize(1000, 650);
        ...
        // Run SJF animation
        btnCalc.addActionListener((ActionEvent e) -> runSJFAnimation());
    }

    private void runSJFAnimation() {
        ...
        new Thread(() -> {
            ...
            while (completed < processList.size()) {
                ...
                ProcessData next = processList.stream()
                        ...
                        .min(Comparator.comparingInt(p -> p.bt))
                        .orElse(null);
                ...
                for (int t = 1; t <= next.bt; t++) {
                    int elapsed = t;
                    int globalTime = startTime + elapsed;

                    SwingUtilities.invokeLater(() -> {
                        cpuLabel.setText("CPU: " + next.pid);
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(() -> new SJF().setVisible(true));
    }
}
